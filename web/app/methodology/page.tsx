    ...
    <main className={styles.page}>
      ...
      <section id="ops" className={styles.section}>
        ...
        <ul>
          ...
          <li>
            Exemplar responses are served from committed JSON snapshots in{" "}
            <code className={styles.code}>data/exemplar_snapshots/</code> so
            every prospect click on one of the five pre-loaded cards (or any
            of the three canned chat starters per card) does not re-spend
            API tokens. The expanded row in{" "}
            <code className={styles.code}>/integrated</code> and the first
            assistant turn in <code className={styles.code}>/chat</code>
            expose a <strong>Re-run live</strong> affordance that bypasses
            the snapshot and hits the live Anthropic API. Free-form pasted
            inputs always run live. Snapshots are regenerated by hand via{" "}
            <code className={styles.code}>
              scripts/regenerate_exemplar_snapshots.py
            </code>{" "}
            after a prompt or model change; the server logs a warning at
            startup if a snapshot&apos;s fingerprint no longer matches the
            current system prompt or tool schema.
          </li>
        </ul>
      </section>
